...
async def on_message(message):
	global embed, tiggen_messages, loyl_messages, current_reader
...
async def show_inbox(ctx):
	global embed, tiggen_messages, loyl_messages
	if ctx.message.author.id == 1131062164866744350:
		# show tiggentime's messages
		if not embed.fields:
			embed.add_field(name="tiggen", value=tiggen_messages, inline=False)
		await ctx.send(embed=embed)
	elif ctx.message.author.id == 689281847469015130:
		# show loyl's messages
		if not embed.fields:
			embed.add_field(name="loyl", value=loyl_messages, inline=False)
		await ctx.send(embed=embed)
